...
const supportResponse = async (req, res, next) => {
  try {
    ...
    const reply = await chatCompletion({ system, user: cleanMessage, temperature: 0.3 });
...
const detectIntent = async (req, res, next) => {
  try {
    ...
    const raw = await chatCompletion({ system, user: cleanMessage, temperature: 0 });
...
const salesReply = async (req, res, next) => {
  try {
    ...
    const reply = await chatCompletion({ system, user: cleanMessage, temperature: 0.4 });
...
const productQA = async (req, res, next) => {
  try {
    const { question, productData } = matchedData(req);
    ...
    const reply = await chatCompletion({ system, user, temperature: 0.2 });
...
const summarize = async (req, res, next) => {
  try {
    const { conversation } = matchedData(req);
    ...
    const reply = await chatCompletion({ system, user: cleanConversation, temperature: 0.2, maxTokens: 256 });
